...
from pyxml.dom import EMPTY_NAMESPACE, EMPTY_PREFIX, XMLNS_NAMESPACE, domreg
from pyxml.dom.minicompat import *
from pyxml.dom.xmlbuilder import DOMImplementationLS, DocumentLS
...
#
_nodeTypes_with_children = (pyxml.dom.Node.ELEMENT_NODE,
                            pyxml.dom.Node.ENTITY_REFERENCE_NODE)


class Node(pyxml.dom.Node, GetattrMagic):
    ...
    def insertBefore(self, newChild, refChild):
        ...
        if newChild.nodeType not in self._child_node_types:
            raise pyxml.dom.HierarchyRequestErr(
...
class Element(Node):
    ...
    def setAttributeNode(self, attr):
        if attr.ownerElement not in (None, self):
            raise pyxml.dom.InuseAttributeErr("attribute node already owned")
...
class CharacterData(Childless, Node):
    ...
    def substringData(self, offset, count):
        if offset < 0:
            raise pyxml.dom.IndexSizeErr("offset cannot be negative")
        if offset > len(self.data):
            raise pyxml.dom.IndexSizeErr("offset cannot be beyond end of data")
        if count < 0:
            raise pyxml.dom.IndexSizeErr("count cannot be negative")
...
class Text(CharacterData):
    ...
    def splitText(self, offset):
        if offset < 0 or offset > len(self.data):
            raise pyxml.dom.IndexSizeErr("illegal offset value")
...
class DocumentType(Identified, Childless, Node):
    ...
    def cloneNode(self, deep):
        if self.ownerDocument is None:
            ...
            operation = pyxml.dom.UserDataHandler.NODE_CLONED
            if deep:
                clone.entities._seq = []
...
class DOMImplementation(DOMImplementationLS):
    ...
    def createDocument(self, namespaceURI, qualifiedName, doctype):
        if doctype and doctype.parentNode is not None:
            raise pyxml.dom.WrongDocumentErr(
        ...
        if not qualifiedName and add_root_element:
            ...
            raise pyxml.dom.InvalidCharacterErr("Element with no name")

        if add_root_element:
            prefix, localname = _nssplit(qualifiedName)
            if prefix == "xml" \
               and namespaceURI != "http://www.w3.org/XML/1998/namespace":
                raise pyxml.dom.NamespaceErr("illegal use of 'xml' prefix")
...
class Document(Node, DocumentLS):
    ...
    def cloneNode(self, deep):
        ...
        self._call_user_data_handler(pyxml.dom.UserDataHandler.NODE_CLONED,
                                     self, clone)
        return clone
    ...
    def importNode(self, node, deep):
        if node.nodeType == Node.DOCUMENT_NODE:
            raise pyxml.dom.NotSupportedErr("cannot import document nodes")
        elif node.nodeType == Node.DOCUMENT_TYPE_NODE:
            raise pyxml.dom.NotSupportedErr("cannot import document type nodes")
    ...
    def renameNode(self, n, namespaceURI, name):
        ...
        if n.nodeType not in (Node.ELEMENT_NODE, Node.ATTRIBUTE_NODE):
            raise pyxml.dom.NotSupportedErr(
                "renameNode() only applies to element and attribute nodes")
        if namespaceURI != EMPTY_NAMESPACE:
            if ':' in name:
                prefix, localName = name.split(':', 1)
                if (  prefix == "xmlns"
                      and namespaceURI != pyxml.dom.XMLNS_NAMESPACE):
                    raise pyxml.dom.NamespaceErr(
...
def _clone_node(node, deep, newOwnerDocument):
    ...
    if node.ownerDocument.isSameNode(newOwnerDocument):
        ...
        operation = pyxml.dom.UserDataHandler.NODE_IMPORTED
    if node.nodeType == Node.ELEMENT_NODE:
        ...
        raise pyxml.dom.NotSupportedErr("Cannot clone node %s" % repr(node))
...
def parse(file, parser=None, bufsize=None):
    ...
    if parser is None and not bufsize:
        from pyxml.dom import expatbuilder
        return expatbuilder.parse(file)
    else:
        from pyxml.dom import pulldom
